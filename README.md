...
<h1 align="center">bluetooth-timeout <code>v0.3.1</code></h1>
...
2.  **Install using Just:**
    ...
    ```sh
    just install                                    # interactive (prompts for config)
    just install --skip-config                      # skip if config exists
...
In debug mode, configuration is read from `.local/config/config.lua` and logs are written to `.local/share/bluetooth-timeout.log`.
